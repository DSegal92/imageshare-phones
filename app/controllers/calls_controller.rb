class CallsController < ApplicationController
  ...
  def show
    ...
    day = Time.now.wday
    if Call.where(:caller_ID => params[:caller_ID]).where(:target => group.identity).where(:created_at => group.callback.days.ago..Time.now).where(:was_connected => true).exists?
