...
When a call comes in from Tropo, the extension and Caller ID are passed to the Rails application. From there, the decision making is as follows:
    If the call's Caller ID matches another recent call (definition of recent controlled by callback variable in groups),
     it is routed to the last person the caller spoke to.
